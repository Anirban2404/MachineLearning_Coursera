...
def featureNormalize(X):
    # FEATURENORMALIZE Normalizes the features in X FEATURENORMALIZE(X) returns a
    # normalized version of X where the mean value of each feature is 0 and the
    # standard deviation is 1.
    # This is often a good preprocessing step to do when working with learning algorithms.
    #
    # You need to set these values correctly
    X_norm = X
    mu = np.zeros((1, X.shape[1]), dtype=np.float)
    sigma = np.zeros((1, X.shape[1]), dtype=np.float)

    # ====================== YOUR CODE HERE ======================
    # Instructions: First, for each feature dimension, compute the mean of
    # the feature and subtract it from the dataset, storing the mean value in mu.
